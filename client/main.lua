...
-- entire structure to prevent mutation, then registers the client export.
--
-- Load order (defined in fxmanifest.lua):
--   shared:  enums → math → utils  (LWEnum, LWMath, LWUtil populated)
--   client:  config → cooldown → main.lua  (LWCooldown populated, then assembled here)
-- ---------------------------------------------------------------------------
...
--- such as LWCooldown.__index = LWCooldown.
---@param  t        table
...
if LW_CONFIG.debug then
    print('[lw-shared] client ready — LWUtils assembled, frozen, and exported')
end